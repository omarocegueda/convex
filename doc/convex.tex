...
\begin{equation}
	\min_{s}\max_{p,q} <s, -div\;p> + <s, T^{*}[q]> -\frac{\lambda}{2}||q + r||_{2}^{2} -\delta_{P}(p)
\end{equation}
...
\begin{equation}
	F^{*}(p, q) = F_{p}^{*}(p) + F_{q}^{*}(q) = \delta_{P}(p) + \frac{\lambda}{2}||q + r||_{2}^{2}
\end{equation}
...
\end{equation}
\begin{equation}
	Prox_{\sigma F_{q}^{*}(\cdot)}(\tilde{p}, \tilde{q}) = \arg\min_{q} \frac{\lambda}{2}||q + r||_{2}^{2} + \frac{1}{2\sigma}||q-\tilde{q}||_{2}^{2}
\end{equation}
...
\begin{equation}
	\sigma\lambda (q+r)+(q-\tilde{q})=0 \Rightarrow q=\frac{1}{1+\sigma\lambda}\left(\tilde{q}-\sigma\lambda r\right).
\end{equation}
...
\begin{equation}
	\left\lbrace\begin{array}{l}
		...
		q^{n+1}(x) = \frac{1}{1+\sigma*\lambda}\left[q^{n}(x) + \sigma T[\bar{s}^{n}](x) - \sigma\lambda r(x)\right],\; \forall x\in \mathcal{L}_F\\
		s^{n+1} = s^{n} - \tau \left[-div\;p^{n+1} + T^{*}[q^{n+1}]\right]\\
		\bar{s}^{n+1} = s^{n+1}+\theta(s^{n+1} - s^{n})
	\end{array}\right.
\end{equation}
