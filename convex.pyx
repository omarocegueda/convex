...
def linprog_pd_small(double[:] c, double[:,:] A, double[:] b, int maxIter, double tolerance):
    ...
    cdef int retVal
    retVal=linprogPDSmall(&c[0], &A[0,0], n, m, &b[0], &x[0], maxIter, tolerance)
    print 'RetVal:',retVal
    return x

def linprog_pd_large(bytes cname, bytes Aname, bytes bname, int n, int m, int maxIter, double tolerance):
    cdef double[:] x=np.zeros(shape=(m,), dtype=np.double)
    cdef int retVal
    retVal=linprogPDLarge(cname, Aname, n, m, bname, &x[0], maxIter, tolerance)
    print 'RetVal:',retVal
    return x
